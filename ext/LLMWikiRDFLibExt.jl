...
function LLMWiki.wiki_to_rdf(config::LLMWiki.WikiConfig; include_provenance::Bool=true)
    ...
    for (slug, (meta, body)) in slug_to_meta
        ...
        # Source provenance — wasDerivedFrom
    ...
    # Add wiki-level provenance from state (richer PROV-O)
    if include_provenance
        state = LLMWiki.load_state(config)

        # Each compile run is a PROV Activity
        ...
        # LLMWiki itself is the PROV Agent (software agent)
        agent_uri = WIKI("agent/llmwiki-compiler")
        add!(g, Triple(agent_uri, RDF.type, PROV.SoftwareAgent))
        add!(g, Triple(agent_uri, RDFS.label, Literal("LLMWiki Compiler")))
        add!(g, Triple(activity_uri, PROV.wasAssociatedWith, agent_uri))
        ...
        for (file, entry) in state.sources
            ...
            # hadPrimarySource — URL provenance for web-ingested sources
            if entry.source_url !== nothing && !isempty(entry.source_url)
                url_uri = URIRef(entry.source_url)
                add!(g, Triple(src_uri, PROV.hadPrimarySource, url_uri))
                add!(g, Triple(url_uri, RDF.type, PROV.Entity))
                add!(g, Triple(url_uri, RDFS.label, Literal(entry.source_url)))
            end

            # alternateOf — PDF→text conversion provenance
            if entry.source_type == "pdf" && entry.original_file !== nothing
                pdf_uri = _source_uri(entry.original_file)
                add!(g, Triple(pdf_uri, RDF.type, PROV.Entity))
                add!(g, Triple(pdf_uri, RDFS.label, Literal(entry.original_file)))
                add!(g, Triple(pdf_uri, DCTERMS.format, Literal("application/pdf")))
                add!(g, Triple(src_uri, PROV.alternateOf, pdf_uri))
                add!(g, Triple(src_uri, PROV.wasDerivedFrom, pdf_uri))
            end

            # Source type as dcterms:format
            if !isempty(entry.source_type) && entry.source_type != "file"
                add!(g, Triple(src_uri, DCTERMS.format, Literal(entry.source_type)))
            end
            ...
            # wasGeneratedBy — each concept was generated by the compilation
            for concept_slug in entry.concepts
                concept_uri = _concept_uri(concept_slug)
                add!(g, Triple(concept_uri, PROV.wasGeneratedBy, activity_uri))
                add!(g, Triple(activity_uri, PROV.generated, concept_uri))
            end
        end

        # Git versioning provenance — link activity to commit hash
        if LLMWiki._has_git(config)
            try
                hash = strip(String(LLMWiki._git_output(config.wiki_dir, ["rev-parse", "HEAD"])))
                if !isempty(hash)
                    revision_uri = WIKI("revision/$hash")
                    add!(g, Triple(revision_uri, RDF.type, PROV.Entity))
                    add!(g, Triple(revision_uri, RDFS.label, Literal(hash)))
                    add!(g, Triple(revision_uri, PROV.wasGeneratedBy, activity_uri))
                    add!(g, Triple(activity_uri, PROV.generated, revision_uri))
                end
            catch; end
        end
    end
